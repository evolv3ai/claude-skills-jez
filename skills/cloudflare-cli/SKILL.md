...
**CLI Version**: v0.0.3 (alpha — standalone Go binary, self-updates via `cf update`)
**Dependencies**: cf binary + Cloudflare account with API token
**Source**: https://github.com/coollabsio/cloudflare-cli
...
- **cloudflare-cli.md** — Correction rules for projects using the cf CLI

Copy to project:
```bash
mkdir -p .claude/rules
cp ~/.claude/skills/cloudflare-cli/rules/cloudflare-cli.md .claude/rules/
```
...
**Required**: cf binary (standalone Go binary)
**Required**: Cloudflare account with API token (Zone:Read + DNS:Read + DNS:Edit)

No npm or package dependencies. The CLI is a standalone binary that self-updates via `cf update`.

---

## Official Documentation

- **Cloudflare CLI GitHub**: https://github.com/coollabsio/cloudflare-cli
...
- [ ] cf binary installed (`cf version` works)
- [ ] API token created with Zone:Read + DNS:Read + DNS:Edit permissions
- [ ] Token saved: `cf auth save <token>`
- [ ] Auth verified: `cf auth verify`
- [ ] Zones accessible: `cf zones list`
- [ ] DNS records visible: `cf dns list <zone>`
- [ ] JSON output tested: `cf dns list <zone> -o json`
